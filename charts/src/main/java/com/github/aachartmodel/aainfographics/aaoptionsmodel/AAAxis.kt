...
open class AAAxis {
    ...
    var lineWidth: Float? = null //坐标轴轴线宽度
    var lineColor: String? = null //坐标轴轴线线颜色
    var max: Float? = null //坐标轴最大值
    var min: Float? = null //坐标轴最小值（设置为0就不会有负数）
    var minRange: Int? = null
    var minTickInterval: Int? =
        null //The minimum tick interval allowed in axis values. For example on zooming in on an axis with daily data, this can be used to prevent the axis from showing hours. Defaults to the closest distance between two points on the axis.
